...
```bash
php artisan vendor:publish --provider="DutchBridge\KlaviyoForLaravel\LaravelKlaviyoServiceProvider" --tag="tags-config"
```
...
If the current user is not identified and `Auth::user()` is an instance of `DutchBridge\KlaviyoForLaravel\Contracts\KlaviyoIdentity` then the `getKlaviyoIdentity` method will be called and an identify
...
is an instance of `DutchBridge\KlaviyoForLaravel\Contracts\KlaviyoIdentity`.
...
```php
use DutchBridge\KlaviyoForLaravel\Klaviyo;
